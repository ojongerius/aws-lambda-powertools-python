...
class IdempotencyHandler:
    ...
    def __init__(
        ...
        persistence_store.configure(config, self.data, self.function.__name__)
    ...
    def _get_idempotency_record(self) -> DataRecord:
        ...
        """
        try:
            data_record = self.persistence_store.get_record()
    ...
    def _get_function_response(self):
        try:
            ...
            try:
                self.persistence_store.delete_record(exception=handler_exception)
            ...
            try:
                self.persistence_store.save_success(result=response)
